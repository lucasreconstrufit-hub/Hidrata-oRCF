...
// v2.0 — desfazer dose, remover quantidade, reset, editar perfil
// ============================================================
...
const ST = {
  ...
  history: [],      // [{ date, meta, turnos, lastDoses }]
  todayEntry: null,
};
...
function ensureToday() {
  ...
  if (!entry) {
    entry = {
      ...
      meta: ST.user.meta,
      turnos: {
        manha: { consumed: 0, done: false, lastDoses: [] },
        tarde: { consumed: 0, done: false, lastDoses: [] },
        noite: { consumed: 0, done: false, lastDoses: [] },
      }
    };
    ST.history.push(entry);
    persist();
  }
  // ensure lastDoses array exists (for older entries)
  TURNOS.forEach(t => {
    if (!entry.turnos[t.id].lastDoses) entry.turnos[t.id].lastDoses = [];
  });
  ST.todayEntry = entry;
}

// ============================================================
// ACTIONS — ADD / UNDO / REMOVE
// ============================================================
function addDose(turnoId, ml) {
  ...
  t.consumed += ml;
  t.lastDoses.push(ml);
  // keep only last 10 doses for undo history
  if (t.lastDoses.length > 10) t.lastDoses.shift();
...
// DESFAZER — remove a última dose registrada no turno
function undoDose(turnoId) {
  if (!ST.todayEntry) return;
  const t = ST.todayEntry.turnos[turnoId];
  if (!t.lastDoses || t.lastDoses.length === 0) {
    toast('Nenhuma dose para desfazer', 'yellow');
    return;
  }
  const last = t.lastDoses.pop();
  t.consumed = Math.max(0, t.consumed - last);
  // recheck done status
  const info  = TURNOS.find(x => x.id === turnoId);
  const tMeta = Math.round(ST.todayEntry.meta * info.pct);
  if (t.consumed < tMeta) t.done = false;
  persist();
  renderToday();
  toast(`↩ ${last}ml removido`, 'yellow');
}

// REMOVER QUANTIDADE — abre sheet para digitar quanto remover
function openRemoveDose(turnoId) {
  const t    = ST.todayEntry.turnos[turnoId];
  const info = TURNOS.find(x => x.id === turnoId);
  openSheet(`
    <div class="sheet-title">Remover Quantidade</div>
    <p style="font-size:13px;color:var(--rcf-muted);margin-bottom:14px">
      Total atual no turno <strong style="color:var(--rcf-white)">${info.icon} ${info.label}</strong>:
      <strong style="color:var(--rcf-red)">${(t.consumed||0).toLocaleString('pt-BR')}ml</strong>
    </p>
    <label>Quanto deseja remover (ml)</label>
    <input type="number" id="remove-ml" placeholder="Ex: 200" min="1" max="${t.consumed || 1}">
    <div class="btn-row">
      <button class="btn-secondary" onclick="closeSheet()">Cancelar</button>
      <button class="btn-primary" style="background:var(--rcf-red)" onclick="removeDose('${turnoId}')">Remover</button>
    </div>
  `);
}

function removeDose(turnoId) {
  const v = parseInt(document.getElementById('remove-ml').value);
  if (!v || v <= 0) { toast('Valor inválido', 'red'); return; }
  const t     = ST.todayEntry.turnos[turnoId];
  const info  = TURNOS.find(x => x.id === turnoId);
  const tMeta = Math.round(ST.todayEntry.meta * info.pct);
  t.consumed  = Math.max(0, t.consumed - v);
  if (t.consumed < tMeta) t.done = false;
  // update lastDoses to reflect removal
  t.lastDoses = [];
  persist();
  closeSheet();
  renderToday();
  toast(`${v}ml removido do turno ${info.label}`, 'yellow');
}
...
function renderHeader() {
  ...
  metaEl.innerHTML = streak >= 2
    ? `🔥 ${streak} dias<br><span style="font-size:9px;color:var(--rcf-muted);letter-spacing:0.06em">seguidos</span>`
...
function renderToday() {
  ...
  document.getElementById('turnos').innerHTML = TURNOS.map((t, idx) => {
    ...
    const hasHistory = te.lastDoses && te.lastDoses.length > 0;
    ...
    // Undo/remove controls — só aparecem se há consumo registrado
    const undoRemoveRow = (te.consumed > 0) ? `
      <div class="undo-row">
        <button class="btn-undo" onclick="undoDose('${t.id}')" ${!hasHistory ? 'disabled' : ''}>
          ↩ Desfazer última
        </button>
        <button class="btn-undo" onclick="openRemoveDose('${t.id}')">
          − Remover quantidade
        </button>
      </div>` : '';
...
function confirmReset() {
  openSheet(`
    <div class="sheet-title" style="color:#FF6666">⚠️ Resetar Dados</div>
    <p style="font-size:14px;color:var(--rcf-muted);line-height:1.6;margin-bottom:20px">
      Isso apagará <strong style="color:var(--rcf-white)">todo o seu histórico e perfil</strong> permanentemente.<br>
      O app voltará para a tela inicial. Esta ação não pode ser desfeita.
    </p>
    <div class="btn-row">
      <button class="btn-secondary" onclick="closeSheet()">Cancelar</button>
